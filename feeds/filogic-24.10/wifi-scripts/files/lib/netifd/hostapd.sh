...
hostapd_common_add_device_config() {
	...
	config_add_bolean maxassoc_ignore_probe
	config_add_int reg_power_type
	config_add_boolean stationary_ap

	config_add_string acs_chan_bias
	config_add_bolean acs_exlude_dfs
	config_add_array hostapd_options

	config_add_int airtime_mode

	config_add_bolean multiple_bssid rnr_beacon he_co_locate ema
...
hostapd_prepare_device_config() {
	...
	json_get_vars country country3 country_ie beacon_int:100 dtim_period:2 doth require_mode legacy_rates \
		...
		band reg_power_type stationary_ap \
...
hostapd_set_bss_options() {
	...
	json_get_vars \
		...
		vendor_elements fils apup uci_section dynamic_probe_resp
